...
parser.add_argument('--dataset', default='VOC', choices=['VOC', 'COCO', 'BABY'],
...
def train():
    if args.dataset == 'COCO':
        ...
        if args.dataset_root == COCO_ROOT:
            parser.error('Must specify dataset if specifying dataset_root')
        cfg = baby
        dataset = BabyDetection(root=args.dataset_root)
    ...
    batch_iterator = cycle(iter(data_loader))
    for iteration in range(args.start_iter, cfg['max_iter']):
        ...
        if args.visdom:
            update_vis_plot(viz, iteration, loss_l.data.item(), loss_c.data.item(),
